...
class UpdateDuckDnstCase(unittest.TestCase):
    ...
    def test1_check_ip_with_fqdn_return_false_when_internet_none(self, logger, MockArcher1200):
        wan_ip = "1.2.3.4"
        internet = None
        expected = False
        MockArcher1200.get_wan_ip = Mock(return_value=wan_ip)
        MockArcher1200.get_internet_status = Mock(return_value=internet)
        res = check_ip_with_fqdn(MockArcher1200, duckdns_fqdn)
        MockArcher1200.logout.assert_called()
        MockArcher1200.get_wan_ip.assert_called()
        MockArcher1200.get_internet_status.assert_called()
        logger.error.assert_called()
        self.assertEqual(expected, res)

    @patch('updateDuckDns.logger')
    def test2_check_ip_with_fqdn_return_false_when_internet_dicsonnected(self, logger, MockArcher1200):
        wan_ip = "1.2.3.4"
    ...
    def test4_check_ip_with_fqdn_return_false_when_wan_ip_equals_duck_ip(self, mock_logger, mock_gethostbyname,
        ...
        mock_gethostbyname.return_value = duck_ip
        res = check_ip_with_fqdn(MockArcher1200, duckdns_fqdn)
        MockArcher1200.get_wan_ip.assert_called_once()
        MockArcher1200.get_internet_status.assert_called_once()
        MockArcher1200.logout.assert_called_once()
        mock_logger.error.assert_not_called()
        mock_logger.debug.assert_called_once_with(f' router ip: {wan_ip} ==  {duckdns_fqdn}: {duck_ip}')
